...
<html>
  ...
  <body>
    ...
    <script>
      // Click-through by default; Alt toggles edit mode.
      ...
      window.addEventListener('keydown', (e) => {
        if (e.key === 'Alt') setEditMode(true);
      }, true);
      window.addEventListener('keyup', (e) => {
        if (e.key === 'Alt') setEditMode(false);
      }, true);
...
function newOverlayWindow(): BrowserWindow {
  ...
  const win = new BrowserWindow({
    ...
    resizable: true,
    movable: true,
    focusable: false, // in click-through mode we don't want focus-stealing
    alwaysOnTop: true,
    skipTaskbar: false,
    webPreferences: {
      sandbox: false,
      contextIsolation: true,
      // No preload needed; we're only using require('electron') in overlay HTML (enable below)
      nodeIntegration: true, // used only for ipcRenderer in overlay HTML
    },
  });

  // Default: ignore mouse events (click-through). Toggle via IPC.
  win.setIgnoreMouseEvents(true, { forward: true }); // click-through (Electron docs)
...
export async function createOverlay(id: string, stickerUrl: string) {
  ...
  // Wire per-window IPC for edit controls
  const { ipcMain } = require('electron');
  ipcMain.handleOnce('overlay:set-click-through', (_e, on: boolean) => {
    try { win.setIgnoreMouseEvents(on, { forward: true }); } catch {}
  });
  ipcMain.handle('overlay:nudge', (_e, dx: number, dy: number) => {
    try {
  ...
  // Load minimal editor HTML that shows the sticker and local edit UI
