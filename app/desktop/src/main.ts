...
// Hosted Login/Library — use first‑party cookies; fall back to local if offline.
...
app.commandLine.appendSwitch('enable-logging');        // Chromium logging
app.commandLine.appendSwitch('log-file', chromiumLog); // write to this file
app.commandLine.appendSwitch('v', '1');                // verbosity

// Optional: register custom protocol for production installers (deep links)
...
app.whenReady()
  .then(() => {
    ...
    // Overlay IPC — includes synthetic event from handleOverlayLink()
